...
namespace ax
{
  ...
  #define DECLARE_KEY_TESTER(Key)                                                 \
    ...
    {                                                                             \
      template<typename T>                                                        \
      static int Get(                                                             \
          typename std::enable_if<has_nested_##Key<ImGuiKey_>::value, T>::type*)  \
      {                                                                           \
...
static void ImDrawListSplitter_Grow(
    ImDrawList* draw_list, ImDrawListSplitter* splitter, int channels_count)
{
...
static void ImDrawListSplitter_SwapChannels(
    ImDrawListSplitter* splitter, int left, int right)
{
...
static void ImDrawList_SwapSplitter(
    ImDrawList* drawList, ImDrawListSplitter& splitter)
{
...
static void ImDrawList_PathBezierOffset(
    ImDrawList* drawList, float offset, const ImVec2& p0, const ImVec2& p1,
...
void ed::Pin::Draw(ImDrawList* drawList, DrawFlags flags)
{
  if (flags & Hovered)
  {
    ...
    if (m_BorderWidth > 0.0f)
    {
      ...
      drawList->AddRect(
          m_Bounds.Min, m_Bounds.Max, m_BorderColor, m_Rounding, m_Corners,
          m_BorderWidth);
    }
...
void ed::Node::Draw(ImDrawList* drawList, DrawFlags flags)
{
  if (flags == Detail::Object::None)
  {
    ...
    {
      drawList->AddRectFilled(
          m_GroupBounds.Min, m_GroupBounds.Max, m_GroupColor, m_GroupRounding);
      ...
      if (m_GroupBorderWidth > 0.0f)
      {
        ...
        drawList->AddRect(
            m_GroupBounds.Min, m_GroupBounds.Max, m_GroupBorderColor,
            m_GroupRounding, c_AllRoundCornersFlags, m_GroupBorderWidth);
      }
    }
  ...
  else if (flags & Selected)
  {
    ...
    DrawBorder(
        drawList, borderColor, editorStyle.SelectedNodeBorderWidth,
  ...
  else if (!IsGroup(this) && (flags & Hovered))
  {
    ...
    DrawBorder(
        drawList, borderColor, editorStyle.HoveredNodeBorderWidth,
        editorStyle.HoverNodeBorderOffset);
  }
}

void ed::Node::DrawBorder(
    ImDrawList* drawList, ImU32 color, float thickness, float offset)
{
  if (thickness > 0.0f)
  {
    ...
    drawList->AddRect(
        m_Bounds.Min - extraOffset, m_Bounds.Max + extraOffset, color,
...
ImRect ed::Node::GetRegionBounds(NodeRegion region) const
{
  ...
  else if (m_Type == NodeType::Group)
  {
    const float activeAreaMinimumSize = ImMax(
        ImMax(
            Editor->GetView().InvScale * c_GroupSelectThickness, m_GroupBorderWidth),
...
bool ed::Link::TestHit(const ImVec2& point, float extraThickness) const
{
  ...
  const auto result =
      ImProjectOnCubicBezier(point, bezier.P0, bezier.P1, bezier.P2, bezier.P3, 50);
...
bool ed::Link::TestHit(const ImRect& rect, bool allowIntersect) const
{
  ...
  if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p0, p1)
          .Count
      > 0)
    return true;
  if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p1, p2)
          .Count
      > 0)
    return true;
  if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p2, p3)
          .Count
      > 0)
    return true;
  if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p3, p0)
          .Count
      > 0)
    return true;
  ...
  if (!ImRect_IsEmpty(previousVisibleRect) && previousSize != canvasSize)
  {
    ...
    if (m_Config.CanvasSizeMode == ax::NodeEditor::CanvasSizeMode::FitVerticalView)
    {
      ...
      width  = height * currentAspectRatio;
        ...
        m_Config.CanvasSizeMode == ax::NodeEditor::CanvasSizeMode::FitHorizontalView)
    {
...
void ed::EditorContext::End()
{
  ...
  auto control = BuildControl(
      m_CurrentAction
      && m_CurrentAction->IsDragging()); // NavigateAction.IsMovingOverEdge()
  // auto& editorStyle = GetStyle();

  m_HoveredNode =
      control.HotNode && m_CurrentAction == nullptr ? control.HotNode->m_ID : 0;
  m_HoveredPin =
      control.HotPin && m_CurrentAction == nullptr ? control.HotPin->m_ID : 0;
  m_HoveredLink =
      control.HotLink && m_CurrentAction == nullptr ? control.HotLink->m_ID : 0;
  m_DoubleClickedNode =
  ...
  // Highlight selected objects
  {
    ...
    for (auto& link : m_Links)
    {
      ...
      auto isLinkHighlighted = isLinkHighlightedForPin(*link->m_StartPin)
  ...
  // node drawing order.
  {
    ...
    auto liveNodeCount = static_cast<int>(std::count_if(
        m_Nodes.begin(), m_Nodes.end(), [](Node* node) { return node->m_IsLive; }));
    ...
    ImDrawList_ChannelsGrow(
        m_DrawList, m_DrawList->_Splitter._Count + c_ChannelsPerNode * liveNodeCount
                        + c_LinkChannelCount);
    ...
    auto groupsItEnd = std::find_if(
        m_Nodes.begin(), m_Nodes.end(), [](Node* node) { return !IsGroup(node); });
...
#if 1 // #FIXME
  {
    ...
    for (float x = fmodf(offset.x, GRID_SX); x < VIEW_SIZE.x; x += GRID_SX)
      m_DrawList->AddLine(
          ImVec2(x, 0.0f) + VIEW_POS, ImVec2(x, VIEW_SIZE.y) + VIEW_POS, GRID_COLOR);
    for (float y = fmodf(offset.y, GRID_SY); y < VIEW_SIZE.y; y += GRID_SY)
      m_DrawList->AddLine(
          ImVec2(0.0f, y) + VIEW_POS, ImVec2(VIEW_SIZE.x, y) + VIEW_POS, GRID_COLOR);
  }
#endif
  ...
  // Draw border
  {
    ...
    m_DrawList->AddRect(
        m_Canvas.Rect().Min + ImVec2(1, 1), m_Canvas.Rect().Max - ImVec2(1, 1),
...
bool ed::EditorContext::DoLink(
    LinkId id, PinId startPinId, PinId endPinId, ImU32 color, float thickness)
{
  // auto& editorStyle = GetStyle();
...
void ed::EditorContext::FindNodesInRect(
    const ImRect& r, vector<Node*>& result, bool append, bool includeIntersecting)
{
  if (!append)
    result.resize(0);
...
void ed::EditorContext::FindLinksInRect(
    const ImRect& r, vector<Link*>& result, bool append)
{
  if (!append)
    result.resize(0);
...
void ed::EditorContext::FindLinksForNode(
    NodeId nodeId, vector<Link*>& result, bool add)
{
  if (!add)
    result.clear();
...
int ed::EditorContext::CountLiveNodes() const
{
  return (int)std::count_if(
      ...
      [](const Node* node) { return node->m_IsLive; });
}

int ed::EditorContext::CountLivePins() const
{
  return (int)std::count_if(
      m_Pins.begin(), m_Pins.end(), [](const Pin* pin) { return pin->m_IsLive; });
}
...
int ed::EditorContext::CountLiveLinks() const
{
  return (int)std::count_if(
      ...
      [](const Link* link) { return link->m_IsLive; });
}
  ...
  for (auto& node : m_Nodes)
  {
    ...
    if (!node->m_RestoreState && settings->m_IsDirty && m_Config.SaveNodeSettings)
    {
      if (m_Config.SaveNode(
              node->m_ID, settings->Serialize().dump(), settings->m_DirtyReason))
        settings->ClearDirty();
    }
  }
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen)
{
  ...
  static auto invisibleButtonEx = [](const char* str_id, const ImVec2& size_arg,
  ...
  auto emitInteractiveArea =
      [&emitInteractiveAreaEx, extraFlags](ObjectId id, const ImRect& rect)
  { return emitInteractiveAreaEx(id, rect, extraFlags); };

  // Check input interactions over area.
  auto checkInteractionsInArea =
      [this, &emitInteractiveArea, &hotObject, &activeObject, &clickedObject,
       &doubleClickedObject](ObjectId id, const ImRect& rect, Object* object)
  {
    if (emitInteractiveArea(id, rect) >= 0)
      clickedObject = object;
    if (!doubleClickedObject && ImGui::IsMouseDoubleClicked(m_Config.DragButtonIndex)
        && ImGui::IsItemHovered())
  ...
  auto isDragging = ImGui::IsMouseDragging(0, 1) || ImGui::IsMouseDragging(1, 1)
  ...
  return Control(
      hotObject, activeObject, clickedObject, doubleClickedObject, isBackgroundHot,
      isBackgroundActive, backgroundClickButonIndex,
...
void ed::EditorContext::ShowMetrics(const Control& control)
{
  ...
  ImGui::Text(
      "Canvas Size: { w=%g h=%g }", canvasRect.GetWidth(), canvasRect.GetHeight());
  ImGui::Text(
      "Mouse: { x=%.0f y=%.0f } global: { x=%g y=%g }", localMousePos.x,
      localMousePos.y, globalMousePos.x, globalMousePos.y);
  ImGui::Text("Live Nodes: %d", liveNodeCount);
  ImGui::Text("Live Pins: %d", livePinCount);
  ImGui::Text("Live Links: %d", liveLinkCount);
  ImGui::Text(
      "Hot Object: %s (%p)", getHotObjectName(),
      control.HotObject ? control.HotObject->ID().AsPointer() : nullptr);
  if (auto node = control.HotObject ? control.HotObject->AsNode() : nullptr)
  {
    ImGui::SameLine();
    ImGui::Text(
        "{ x=%g y=%g w=%g h=%g }", node->m_Bounds.Min.x, node->m_Bounds.Min.y,
        node->m_Bounds.GetWidth(), node->m_Bounds.GetHeight());
  }
  ImGui::Text(
      "Active Object: %s (%p)", getActiveObjectName(),
...
bool ed::Settings::Parse(const std::string& string, Settings& settings)
{
  ...
  auto deserializeObjectId = [](const std::string& str)
  {
    ...
    auto idStart =
        str.c_str() + ((separator != std::string::npos) ? separator + 1 : 0);
...
ed::NavigateAnimation::NavigateAnimation(
    EditorContext* editor, NavigateAction& scrollAction)
    : Animation(editor)
    , Action(scrollAction)
{
}
...
void ed::NavigateAnimation::OnUpdate(float progress)
{
  ImRect current;
  current.Min =
      ImEasing::EaseOutQuad(m_Start.Min, m_Target.Min - m_Start.Min, progress);
  current.Max =
      ImEasing::EaseOutQuad(m_Start.Max, m_Target.Max - m_Start.Max, progress);
...
void ed::FlowAnimation::Flow(
    ed::Link* link, float markerDistance, float speed, float duration)
{
  Stop();
...
{
  return m_Path.size() > 1 && m_PathLength > 0.0f && m_Link->m_Start == m_LastStart
         && m_Link->m_End == m_LastEnd;
}
...
void ed::FlowAnimationController::Flow(Link* link, FlowDirection direction)
{
  ...
  animation->Flow(
      link, editorStyle.FlowMarkerDistance, editorStyle.FlowSpeed * speedDirection,
  ...
  if (false)
  {
    ...
    if (!Editor->GetSelectedObjects().empty())
    {
          ...
          || (zoomMode != NavigateAction::ZoomMode::None))
      {
        m_LastSelectionId = Editor->GetSelectionId();
        NavigateTo(
            Editor->GetSelectionBounds(), zoomMode, -1.0f,
    ...
    if (navigateToContent)
      NavigateTo(
          Editor->GetContentBounds(), NavigateAction::ZoomMode::WithMargin, -1.0f,
...
bool ed::NavigateAction::HandleZoom(const Control& control)
{
  ...
  if (!io.MouseWheel
      || (!allowOffscreen
          && !Editor->IsHoveredWithoutOverlapp())) // && !ImGui::IsAnyItemActive())
    return false;
...
void ed::NavigateAction::ShowMetrics()
{
  ...
  ImGui::Text(
      "    Visible Rect: { l=%g t=%g, r=%g b=%g w=%g h=%g }", m_VisibleRect.Min.x,
      m_VisibleRect.Min.y, m_VisibleRect.Max.x, m_VisibleRect.Max.y,
      m_VisibleRect.Max.x - m_VisibleRect.Min.x,
      m_VisibleRect.Max.y - m_VisibleRect.Min.y);
}

void ed::NavigateAction::NavigateTo(
    const ImRect& bounds, ZoomMode zoomMode, float duration, NavigationReason reason)
{
  if (ImRect_IsEmpty(bounds))
    return;
...
void ed::NavigateAction::NavigateTo(
    const ImRect& target, float duration, NavigationReason reason)
{
  m_Reason = reason;
...
bool ed::NavigateAction::MoveOverEdge(const ImVec2& canvasSize)
{
  ...
  const auto screenRect =
      ImRect(ImGui::GetCursorScreenPos(), ImGui::GetCursorScreenPos() + canvasSize);
  ...
  const auto offset =
      ImMin(ImMax(screenPointOnEdge - screenMousePos, minDistance), maxDistance);
...
float ed::NavigateAction::GetNextZoom(float steps)
{
  ...
  else
  {
    ...
    return MatchZoom(
        fixedSteps, m_ZoomLevels[fixedSteps < 0 ? 0 : m_ZoomLevelCount - 1]);
  }
}
...
void ed::SizeAction::ShowMetrics()
{
  ...
  ImGui::Text("    Active: %s", m_IsActive ? "yes" : "no");
  ImGui::Text(
      "    Node: %s (%p)", getObjectName(m_SizedNode),
      m_SizedNode ? m_SizedNode->m_ID.AsPointer() : nullptr);
  if (m_SizedNode && m_IsActive)
  {
    ImGui::Text(
        "    Bounds: { x=%g y=%g w=%g h=%g }", m_SizedNode->m_Bounds.Min.x,
        m_SizedNode->m_Bounds.Min.y, m_SizedNode->m_Bounds.GetWidth(),
        m_SizedNode->m_Bounds.GetHeight());
    ImGui::Text(
        "    Group Bounds: { x=%g y=%g w=%g h=%g }",
        m_SizedNode->m_GroupBounds.Min.x, m_SizedNode->m_GroupBounds.Min.y,
        m_SizedNode->m_GroupBounds.GetWidth(),
        m_SizedNode->m_GroupBounds.GetHeight());
    ImGui::Text(
        "    Start Bounds: { x=%g y=%g w=%g h=%g }", m_StartBounds.Min.x,
        m_StartBounds.Min.y, m_StartBounds.GetWidth(), m_StartBounds.GetHeight());
    ImGui::Text(
        "    Start Group Bounds: { x=%g y=%g w=%g h=%g }", m_StartGroupBounds.Min.x,
        m_StartGroupBounds.Min.y, m_StartGroupBounds.GetWidth(),
...
void ed::DragAction::ShowMetrics()
{
  ...
  ImGui::Text("    Active: %s", m_IsActive ? "yes" : "no");
  ImGui::Text(
      "    Node: %s (%p)", getObjectName(m_DraggedObject),
...
ed::EditorAction::AcceptResult ed::SelectAction::Accept(const Control& control)
{
      ...
      && ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex, 1))
  {
    ...
    if (io.KeyCtrl)
      ...
      control.BackgroundClickButtonIndex == Editor->GetConfig().SelectButtonIndex)
  {
    Editor->ClearSelection();
  }
  else
...
bool ed::SelectAction::Process(const Control& control)
{
  ...
  if (ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex, 0))
  {
    ...
    auto bottomRight = ImVec2(
        ImMax(m_StartPoint.x, m_EndPoint.x), ImMax(m_StartPoint.y, m_EndPoint.y));
    ...
    else
    {
      ...
      if (m_SelectGroups)
      {
        auto endIt = std::remove_if(
            m_CandidateObjects.begin(), m_CandidateObjects.end(),
    ...
    m_CandidateObjects.insert(
        m_CandidateObjects.end(), m_SelectedObjectsAtStart.begin(),
...
void ed::SelectAction::Draw(ImDrawList* drawList)
{
  ...
  const auto fillColor = Editor->GetColor(
      m_SelectLinkMode ? StyleColor_LinkSelRect : StyleColor_NodeSelRect, alpha);
  const auto outlineColor = Editor->GetColor(
      m_SelectLinkMode ? StyleColor_LinkSelRectBorder : StyleColor_NodeSelRectBorder,
      alpha);
  ...
  if (isPressed || isReleased || isDragging)
  {
    ...
    if (isPressed)
    {
      ...
      return Possible;
        ...
        isReleased && m_CandidateMenu == candidateMenu && m_ContextId == contextId)
    {
    ...
    if (candidateAction != Paste && candidateAction != CreateNode)
    {
      ...
      if (!selection.empty())
      {
        ...
        if (!extra.empty())
        {
          ...
          m_Context.erase(
              std::unique(m_Context.begin(), m_Context.end()), m_Context.end());
        }
      }
      else if (
      ...
      if (!hasAnyLinks
          && m_Context.size() > 1) // one node cannot make connection to anything
      {
        ...
        auto isNodeInContext = [&nodes](NodeId nodeId)
        {
          return std::binary_search(
              nodes.begin(), nodes.end(), ObjectWrapper<Node>{nodeId, nullptr});
        };
...
ed::EditorAction::AcceptResult ed::DeleteItemsAction::Accept(const Control& control)
{
  ...
  auto& io  = ImGui::GetIO();
  bool keys = ImGui::IsKeyPressed(ImGuiKey_Delete)
              || ImGui::IsKeyPressed(ImGuiKey_Backspace);
  if (Editor->CanAcceptUserInput() && keys && Editor->AreShortcutsEnabled())
  {
    auto& selection = Editor->GetSelectedObjects();
    if (!selection.empty())
    {
  ...
  if (m_CurrentNode->m_CenterOnScreen)
  {
    ...
    if (ImLengthSqr(offset) > 0)
    {
      ...
      else
      {
        ...
        Editor->MakeDirty(
            SaveReasonFlags::Position | SaveReasonFlags::User, m_CurrentNode);
      }
    }
...
void ed::NodeBuilder::End()
{
  ...
  {
    IM_ASSERT(
        drawList->_Splitter._Count
        == 1); // Did you forgot to call drawList->ChannelsMerge()?
    ImDrawList_SwapSplitter(drawList, m_Splitter);
  }
      ...
      || editorStyle.NodePadding.z != 0 || editorStyle.NodePadding.w != 0)
  {
    ...
    ImGui::Dummy(
        ImVec2(0, 0)); // bump cursor at the end of the line and move to next one
...
bool ed::HintBuilder::Begin(NodeId nodeId)
{
  ...
  const auto alpha = ImMax(
      0.0f, std::min(1.0f, (view.Scale - c_min_zoom) / (c_max_zoom - c_min_zoom)));
...
bool ed::Config::SaveNode(
    NodeId nodeId, const std::string& data, SaveReasonFlags flags)
{
  if (SaveNodeSettings)
